...
def train(args: argparse.Namespace, logger: logging.Logger) -> None:
    ...
    training_args = get_orpo_training_args(args)
    train_dataset, val_dataset = get_dataset(
        args.po_train_filepath, args.po_val_filepath
    )
...
def test(args: argparse.Namespace, logger: logging.Logger) -> None:
    ...
    for prompt in tqdm(df["prompt"].values.tolist(), leave=True):
        ...
        predictions_po.append(gen_text)

    # Save results ---------------------------
    df["prediction_po"] = predictions_po
    df.to_csv(f"{args.results_dir}/po_predictions.csv", index=False)

    # Get results ---------------------------
    logger.info("Results of llm-po:")
    df = pd.read_csv(f"{args.results_dir}/po_predictions.csv", encoding="utf-8")
    preds: list[list[str]] = []
    refs: list[list[str]] = []
    for pred, ref in zip(
        df["prediction_po"].values.tolist(), df["findings"].values.tolist()
    ):
        preds.append([pred])
        refs.append([ref])
