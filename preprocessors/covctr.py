...
class PreprocessCovctrDataset:
    ...
    def extract_img_features(self, image_path: str) -> np.array:
        ...
        img = Image.open(f"datasets/covctr/images/{image_path}").convert("RGB")
    ...
    def preprocess(self) -> None:
        ...
        train["img_feats"] = train["image_id"].progress_apply(self.extract_img_features)
        val["img_feats"] = val["image_id"].progress_apply(self.extract_img_features)
        test["img_feats"] = test["image_id"].progress_apply(self.extract_img_features)
...
def main() -> None:
    ...
    PreprocessCovctrDataset(
        ...
        "datasets/covctr/test_p.csv",
        "datasets/covctr/sft_train.csv",
        "datasets/covctr/sft_val.csv",
        "datasets/covctr/sft_test.csv",
        "datasets/covctr/po_train.csv",
        "datasets/covctr/po_val.csv",
        "datasets/covctr/po_test.csv",
    ).preprocess()
